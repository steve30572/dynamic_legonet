...
class LegoCNN(nn.Module):
  def __init__(self,ina,out,kernel_size,split,lego,weight):
    ...
    weight=weight.cuda()
    self.first_filter=nn.Parameter(nn.init.kaiming_normal_(weight))
    ...
    self.weight2=torch.cat((self.first_filter,self.first_filter2),dim=0)
    #self.weight=self.weight.cuda()
...
cfg={
    ...
    'vgg16_lego':[16,16,32,'A',64,'A',128,'A',256,512,'A'],     
}
